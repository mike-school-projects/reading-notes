...
------------------------------------------------------------------------------------------------ */
const toLastNames = people => people.map((element) => (element.firstName + ' ' + element.lastName));

// console.log(toLastNames([{ firstName: "Jane", lastName: "Doe" }, { firstName: "James", lastName: "Bond" }]));
...
const addPurchases = (arr) => arr.reduce((acc, element) => (acc + element.purchasePrice), 0);
...
const reversedString = (str) => {
  ...
  tempArray = tempArray.reduce((acc, element) => element + acc), 0;
  return tempArray;
};

// console.log(reversedString('abce'));
...
const countNumberOfChildren = (arr) => {
  // console.log(arr);
  let charactersWithKids = [];
  let childrenArray = [];
  let kidCount = 0;

  // get array of characters with children
  charactersWithKids = arr.filter((element) => element.children);

  // get array of children
  childrenArray = charactersWithKids.map((element) => element.children);

  // count children
  childrenArray.forEach((element) => {
    kidCount += element.reduce((acc) => acc + 1, 0);
  });
  return kidCount;
};
...
const countPrimeNumbers = (arr) => {
  // filter to array of prime numbers
  let primeNumbers = arr.filter((element) => isPrime(element));

  // reduce to count indexes
  return primeNumbers.reduce((acc) => acc + 1, 0);
};
...
const extractStats = (snorlaxData) => {
  // console.log(snorlaxData.stats);
  let tempArray = [];

  tempArray = snorlaxData.stats.reduce((acc, element) => {
...
const extractChildren = (arr) => {
  // filter for characters with 'A' and has children
  let characterA = arr.filter((element) => element.name.includes('a') && element.children);

  // create array of array of children
  let childrenArr = [];
  characterA.forEach((element) => childrenArr.push(element.children));

  // create single array of children
  let childrenList = [];
  childrenList = childrenArr.reduce((acc, element) => {
    element.forEach((element) => acc.push(element));
    return acc;
  }, []);
  return childrenList;
};

...
const raisedToTheThird = (arr) => {
  ...
  arr.forEach((element, index) => {
    tempArray[index] = Math.pow(element, 3);
  });
  return tempArray;
};
...
// const addOne = (arr) => {
//   let tempArray = [];
//   tempArray = arr.map((element) => element + 1);
//   return tempArray;
// };

const addOne = (arr) => arr.map((element) => element + 1);
...
const forEachTwoToThe = (arr) => {
  ...
  arr.forEach((element, i) => {
    tempArray[i] = Math.pow(2, element);
  });
  return tempArray;
};
...
const charCode = (arr) => arr.map((element) => element.charCodeAt());
...
const evenOdd = (arr) => arr.map((element) => {
  if (typeof (element) !== 'number') {
    return 'N/A';
  } else if (element % 2 === 0) {
    return 'even';

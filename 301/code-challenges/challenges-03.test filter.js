...
const addTwo = (arr) => arr.map((element)=> element+2);
...
const oddValues = (arr) => arr.filter((element) => element % 2 === 1);
...
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter((element) => (element.baseStat > minBaseStat));
...
const getStatName = (arr, minBaseStat) => {
  let baseStats = getBaseStatGreaterThan(arr, minBaseStat);
  let tempArray = [];
  baseStats.forEach((element, index) => {
    tempArray[index] = element.stat.name;
  });
  return tempArray;
};
...
const getCharactersWithoutChildren = (arr) => arr.filter((element) => (element.children === undefined));
...
const evenOddNumericValues = (arr) => {
  let tempArray = arr.filter((element) => (typeof (element) === 'number'));
